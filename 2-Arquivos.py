...
Lendo um arquivo inteiro
    Para começar, vamos aprender ler um arquivo. Primeiro crie um arquivo no diretório. Usarei de exemplo 2-Arquivos.txt 
...
read()
    Depois que tivermos um objeto arquivo que represente 2-Arquivos.txt, usamos o método read() na seunda linhas de nosso programa para ler todo o conteúdo do arquivo e armazená-lo em uma longa string em #conteudo."""

print("Exemplo 1")

with open("8-Arquivos_Exceções/2-Exemplo_1.txt") as objeto_arquivo:
    conteudo = objeto_arquivo.read()
    print(conteudo)

"""
    A única diferença entre essa saída e o arquivo original é a linhas em branco extra no final da saída. A linhas em branco aparece porque read() devolve uma string vazia quando alcança o final do arquivo. Se quiser remover essa linhas em branco extra, a função rstrip() pode ser usada na instrução print."""

print("\nExemplo 2")

with open("8-Arquivos_Exceções/2-Exemplo_1.txt") as objeto_arquivo_2:
...
r"""A letra #r foi inserida para que nçao seja interpretado \U como um unicode, o que geraria um erro na chamada.

Lendo dados linha a linha
    Quando estiver lendo um aruivo, com frequência você vai querer analisar cada linhas do arquivo. Talvez você queira procurar uma determinada informação no arquivo ou queira modificar o texto do arquivo de alguma maneira. Para isso podemos usar um laço #for no objeto arquivo ara analisar cada uma de suas linhas, uma de cada vez.
    As linhas em branco aparecem sempre que há o final de uma linha, porque há um caracteres inviível de quebra de linha no de cada uma no arquivo-texto. Para evitarmos que a instrução print sempre adicione uma linha em branco, não esqueça de utilizar a função rstrip(). Experimente retirá-la para ver o que acontece."""

print("\nExemplo 4")
arquivo_nome_4 = "8-Arquivos_Exceções/2-Exemplo_1.txt"
...
arquivo_nome_5 = "8-Arquivos_Exceções/2-Exemplo_1.txt"
...
"""
    Em #a o método readlines() armazena cada linha do arquivo em uma lista. Essa lista é então armazenada em #lines, com a qual podemos conteinuar trabalhando depois que o bloco #with terminar.
    ...
    *Importante: Quando Pythin lê um arquivo-texto, todo o arquivo é interpretado como uma string. Se você ler um número e quiser trabalhar com esse valor em um contexto numérico, será necessário convertê-lo em um número inteiro usando a função int() ou para um flutuante com a função float()"""
